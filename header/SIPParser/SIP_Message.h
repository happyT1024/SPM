...
public:
  Ds_SIP_Message(const std::string &str) { parse(str); }
  ...
  public:
    Ds_name_address name_address;
    std::string tag;
    std::string generics;

    std::string to_str() override {
      std::string answer = "From: " + name_address.to_str();
      if (!tag.empty()) {
        answer += ";tag=" + tag;
      }
      if (!generics.empty()) {
    ...
    void parse(const std::string &str) override {
      ...
      static const boost::regex from_pattern(
          ...
          "^From:\\s*" // Начало и ": "
          "(?:(\"[^\"]*\")|([^<;]+))?\\s*" // Отображаемое имя (в кавычках или
                                           // без)
      ...
      if (boost::regex_search(str, matches, from_pattern)) {
        ...
        if (!quoted_name.empty()) {
          quoted_name = quoted_name.substr(1, quoted_name.length() -
                                                  2); // Удаляем кавычки
        }
        boost::trim(unquoted_name);

        std::string display_name =
            !quoted_name.empty() ? quoted_name : unquoted_name;

        if (!display_name.empty()) {
          this->name_address.parse(display_name + " <" + uri + ">");
        } else {
          this->name_address.parse(str); // Если ничего не распарсилось
        }
      } else {
        // Если не соответствует формату, сохраняем как есть
        this->name_address.parse(str);
      }
    }
  };
  ...
  public:
    std::string to_str() override {
      std::string answer = "To: " + name_address.to_str();
      if (!tag.empty()) {
        answer += ";tag=" + tag;
      }
      if (!generics.empty()) {
  ...
  std::string to_str() { return begin + from.to_str() + to.to_str() + end; }
